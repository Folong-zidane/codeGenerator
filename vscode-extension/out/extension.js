...
function activate(context) {
    ...
    // Commande: Generate
    context.subscriptions.push(vscode.commands.registerCommand('basiccode.generate', async () => {
    ...
    // Commande: Configure
    context.subscriptions.push(vscode.commands.registerCommand('basiccode.configure', async () => {
        await configurationWizard();
    }));
    // Commande: Watch Mode Toggle
    context.subscriptions.push(vscode.commands.registerCommand('basiccode.toggleWatch', async () => {
        if (watcher) {
            disableWatchMode();
        }
        else {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('basiccode.generateWithCommit', async () => {
        await generateWithGitCommit();
    }));
}
exports.activate = activate;
// Welcome Screen
async function showWelcomeScreen(context) {
    const hasSeenWelcome = context.globalState.get('hasSeenWelcome');
    if (!hasSeenWelcome) {
        const choice = await vscode.window.showInformationMessage('🎉 Welcome to basicCode Generator!', 'Quick Start', 'Configure', 'Later');
...
function enableWatchMode() {
    ...
    watcher.onDidChange(async (uri) => {
        const choice = await vscode.window.showInformationMessage(`📝 Diagram changed: ${path.basename(uri.fsPath)}`, 'Regenerate', 'Ignore');
...
async function generateWithGitCommit() {
    await vscode.commands.executeCommand('basiccode.generate');
    const choice = await vscode.window.showInformationMessage('✅ Project generated! Commit changes?', 'Commit', 'Skip');
    if (choice === 'Commit') {
...
class ProjectGenerator {
    ...
    async generate() {
        try {
            ...
            vscode.window.showErrorMessage(`❌ Generation failed: ${message}`, 'Retry', 'Check Backend', 'View Logs').then(choice => {
                if (choice === 'Retry') {
    ...
    async validateBeforeGeneration() {
        ...
        if (Object.keys(diagrams).length === 0) {
            log('No diagrams found', 'warn');
            const choice = await vscode.window.showWarningMessage('⚠️ No diagrams found in src/diagrams/', 'Create Sample', 'Open Folder', 'Cancel');
            if (choice === 'Create Sample') {
                ...
                await vscode.commands.executeCommand('revealFileInOS', vscode.Uri.file(diagramsPath));
                return false;
            }
            return false;
        }
        ...
        try {
            const response = await axios_1.default.get(`${backend}/actuator/health`, {
                timeout: 5000
            });
            ...
            const choice = await vscode.window.showErrorMessage('❌ Backend unreachable. Check your configuration.', 'Configure', 'Use Production', 'Continue Anyway');
            if (choice === 'Configure') {
                await vscode.commands.executeCommand('workbench.action.openSettings', 'basiccode.backend');
                return false;
            }
            else if (choice === 'Use Production') {
                await this.config.update('backend', 'https://codegenerator-cpyh.onrender.com', vscode.ConfigurationTarget.Workspace);
    ...
    User "1" --> "*" Post`;
        await fs.promises.writeFile(path.join(diagramsPath, 'class-diagram.mmd'), sampleDiagram);
        log('Sample project created', 'info');
        vscode.window.showInformationMessage('✅ Sample project created!', 'Generate Now').then(choice => {
